...
int main()
{
    ...
    while(1)
    {
    	...
    	/*after 5 minute for now - when the game will be over
    	 * we set up appropriate the time in the definition*/
    	if(asTimeStruct.MinutesLSB == (TIME_TO_DRAW+1) && InfoFlag == 0){
    			ShowTheInfo(remis);
    	}
    	else{
    		if(InfoFlag==1){ /*FUCK OFF OD MOICH FLAG! jak inni moga rozwieszac na pomnikach to czemu ja tutaj nie moge?*/
    			InitGame();
    			InfoFlag=0;
    		}
